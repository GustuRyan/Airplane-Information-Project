public class Sorting {
    ...
    public static void quickSort(String[] arr, int low, int high) { // O(1) -> konstan (bergantung pada if (low < high)
        if (low < high) {
            int partitionIndex = partition(arr, low, high); // Tpartition(n)

            // Rekursif untuk mengurutkan bagian kiri dan kanan dari pivot
            quickSort(arr, low, partitionIndex - 1); // Tquicksort(n/2)
            quickSort(arr, partitionIndex + 1, high); // Tquicksort (n/2)
        }
    }

    private static int partition(String[] arr, int low, int high) {
        String pivot = arr[high]; // 1
        int i = low - 1; // 1 + 1
